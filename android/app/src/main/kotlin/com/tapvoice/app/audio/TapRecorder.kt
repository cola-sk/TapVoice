...
data class SavedRecording(val buttonId: String, val path: String, val durationMs: Long)
...
class TapRecorder(private val context: Context) {
    ...
    fun start(targetButtonId: String) {
        cancel()
    ...
    fun stop(): SavedRecording? {
        ...
        val destination = File(source.parentFile, "$target.m4a")
        try {
        ...
        buttonId = null
        paused = false
        return SavedRecording(target, destination.absolutePath, duration)
    }
